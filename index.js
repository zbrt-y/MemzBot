...
const asw1 = 'https://ibb.co/fGTx5Nt'
...
module.exports = Memz = async (Memz, mek) => {
  try {
...
const body = (type === 'listResponseMessage' && mek.message.listResponseMessage.title) ? mek.message.listResponseMessage.title : (type === 'buttonsResponseMessage' && mek.message.buttonsResponseMessage.selectedButtonId) ? mek.message.buttonsResponseMessage.selectedButtonId : (type === 'conversation' && mek.message.conversation.startsWith(prefix)) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption.startsWith(prefix) ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption.startsWith(prefix) ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text.startsWith(prefix) ? mek.message.extendedTextMessage.text : (type == 'stickerMessage') && (getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== null && getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== undefined) ? getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) : ""
    ...
    const botNumber = Memz.user.jid;
    
    const botNumberss = Memz.user.jid + "@c.us";
...
// 𝘼𝙪𝙩𝙤 𝙍𝙚𝙖𝙙 𝙂𝙘
var ampun = await Memz.chats.array.filter(v => v.jid.endsWith('g.us'))
ampun.map( async ({ jid }) => {
if (readGc === false) return
await Memz.chatRead(jid)
})
// 𝘼𝙪𝙩𝙤 𝙍𝙚𝙖𝙙 𝙋𝙘
var chatss = await Memz.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
await Memz.updatePresence(from, Presence.recording)
} else if (autoketik) {
	if (autoketik === false) return
await Memz.updatePresence(from, Presence.composing)
}
...
const checkRegisteredUser = (sender) => {
    ...
    let senderr = mek.key.fromMe
      ...
      ? mek.participant
      : mek.key.remoteJid;
    const totalchat = await Memz.chats.all();
    const ownerNumber = [`919605243185@s.whatsapp.net`]
	const senderNumber = sender.split("@")[0] 
    const m = simple.smsg(Memz, mek);
    const groupMetadata = isGroup ? await Memz.groupMetadata(from) : "";
   ...
   const isRegister = register.includes(sender)
    ...
    const conts = mek.key.fromMe
      ? Memz.user.jid
      : Memz.contacts[sender] || { notify: jid.replace(/@.+/, "") };
...
cmhit.push(command)
  ...
  /*const reply = (teks) => {
      Memz.sendMessage(from, teks, text, { quoted: mek });
    };*/
    const textImg = (teks) => {
           return ikyy.sendMessage(from, teks, text, {quoted: freply, thumbnail: fs.readFileSync('./Memz.jpg')})
        }
const math = (teks) => {
           return Math.floor(teks)
       }
/*const reply = (teks) => {
			Memz.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/919605243185?text=hello`}}})
		}*/
    const sendMess = (hehe, teks) => {
      Memz.sendMessage(hehe, teks, text);
    };

    const mentions = (teks, memberr, id) => {
      id == null || id == undefined || id == false
        ? Memz.sendMessage(from, teks.trim(), extendedText, {
...
const freply = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { "contactMessage": { "displayName": `${pushname}`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${senderr.split('@')[0]}:${senderr.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, "jpegThumbnail":fs.readFileSync('./Memz.jpg')
        }}}
...
const ftex = {
    ...
    const fakestatus = (teks) => {
      Memz.sendMessage(from, teks, text, {
        quoted: {
          key: {
            fromMe: false,
            participant: `0@s.whatsapp.net`,
    ...
    const fakethumb = (teks, yes) => {
      Memz.sendMessage(from, teks, image, {
...
Memz.on('CB:action,,battery', json => {
...
download(url, "./stik" + names + ".png", async function () {
        ...
        exec(
          ...
          (err) => {
            ...
            Memz.sendMessage(to, media, MessageType.sticker, { quoted: mek });
...
if (budy.toLowerCase() === `8473`){
		if (isRegister) return 
		    ...
		    Memz.sendMessage(from, teks, text, {quoted: fkontak })
...
key: {
    ...
    const sendMediaURL = async (to, url, text = "", mids = []) => {
      ...
      download(url, filename, async function () {
        ...
        Memz.sendMessage(to, media, type, {
          quoted: mek,
          mimetype: mime,
          caption: text,
...
Memz.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await Memz.prepareMessage(from, kma, image)
...
Memz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Video
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await Memz.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await Memz.prepareMessage(from, kma, location)
const buttonMessages = {
...
Memz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const kick = function (from, orangnya) {
      for (let i of orangnya) {
        Memz.groupRemove(from, [i]);
      }
    };
    const add = function (from, orangnya) {
      Memz.groupAdd(from, orangnya);
    };
        const grupinv = (teks) => {
        	grup = Memz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Memz Bot Inc.`, "footerText": "*_Memz Bot_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
            Memz.relayWAMessage(grup)
        }
const fakeitem = (teks) => {
            Memz.sendMessage(from, teks, text, {
                quoted: {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`Memz Bot`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
             res = Memz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 66666666, "message": teks, "footerText": "*Memz*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
             Memz.relayWAMessage(res)
        }
const fakewa = (teks) => {        
Memz.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Memz`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/919605243185?text=hello`}}})
		}
const replyfakelink = (teks) => {
Memz.sendMessage(from, teks, text,{contextInfo :{text: 'hi',
"forwardingScore": 1000000000,
isForwarded: false,
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\nMemz Bot. (@klsiy) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
                "thumbnailUrl": "https://ibb.co/fGTx5Nt",
                "thumbnail": fake,
                "sourceUrl": ``
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/919605243185`,
                "mediaType": "10",
                "mediaUrl": `wa.me/919605243185`,
                "thumbnailUrl": "https://ibb.co/fGTx5Nt",
                "thumbnail": iye,
                "sourceUrl": `wa.me/919605243185`,
...
"mediaType": "2",
"thumbnailUrl": "https://ibb.co/fGTx5Nt",
"mediaUrl": "https://instagram.com/klsiy",
...
// Support Di Versi 3.5.2 
    const sendBug = async (target, teks) => {
      if (!teks) teks = ".";
      await Memz.relayWAMessage(
        Memz.prepareMessageFromContent(
          target,
          Memz.prepareDisappearingMessageSettingContent(0),
          {}
        ),
        { waitForAck: true }
      );
      Memz.sendMessage(target, teks, "conversation");
    };
...
async function sendFileFromUrl(from, url, caption, msg, men) {
            ...
            return Memz.sendMessage(from, await getBuffer(url), type, {caption: caption, quoted: mek, mimetype: mime, contextInfo: {"mentionedJid": men ? men : []}})
        }
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "919605243185@s.whatsapp.net";
    ...
    //auto voice note by Memz
    for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					Memz.sendMessage(from, result, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
					}
			}
			//auto sticker
			for (let anji of setik){
				if (budy === anji){
					...
					Memz.sendMessage(from, result, sticker, { quoted: mek})
					}
			}
			//auto image
			for (let anjh of imagi){
				if (budy === anjh){
					...
					Memz.sendMessage(from, result, image, { quoted: mek})
					}
			}
			// auto text 
...
await Memz.setStatus(`Memz Bot working since ${uptime}`).catch((_) => _);
...
const serialUser = createSerial(18)
	         try {
								ppimg = await Memz.getProfilePicture(`${sender.split('@')[0]}@c.us`)
								} catch {
...
*Thank you for registering in the Memz Bot Database*
...
*「 Memz Bot 」*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `</️MENU`},type:1}]
              imageMsg = (await Memz.prepareMessageMedia(haitod, "imageMessage", { thumbnail: haitod, })).imageMessage
              buttonsMessage = {footerText:'Memz', imageMessage: imageMsg,
              contentText:`${anuu}`,buttons,headerType:4}
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
              Memz.relayWAMessage(prep)
	         console.log(color('[REGISTER]'), color(time, 'yellow'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
	    // console.log(e)
            setTimeout( () => {
			Memz.updatePresence(from, Presence.composing)
			reply(`*Thank you for being registered Memz Bot*`)
		}, 2000)
        break
      
case 'help':
case 'menu':
groups = Memz.chats.array.filter(v => v.jid.endsWith('g.us'))
              privat = Memz.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
              ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
           
              uptime = process.uptime();
              timestampe = speed();
              totalChat = await Memz.chats.all()
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       txtt =`Hi ${pushname} 👋\n\nIF YOU ARE USING MOD WHATSAPP & BUTTONS DOESNT SHOW, TYPE #allmenu`
               ...
               imageMsg = (await Memz.prepareMessageMedia(fs.readFileSync(`./stik/oksip.jpeg`), 'imageMessage', {thumbnail: fs.readFileSync(`./stik/oksip.jpeg`)})).imageMessage
...
groups = Memz.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "919605243185@s.whatsapp.net"
        dtod = '919605243185'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
       stst = await Memz.getStatus(`${sender.split('@')[0]}@c.us`)
...
case 'credit':
 ...
 🌿 : wa.me/${dtod2.split('@')[0]}
 `
     credit = Memz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 1000, "message": `${yez}`, "footerText": "hehe", "thumbnail": iye, "surface": 'CATALOG'}}, {quoted: mek})
            Memz.relayWAMessage(credit)
        break
...
case 'bugreport':
              ...
              Memz.sendMessage('919605243185@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                ...
                Memz.sendMessage(from, dapuhy, image, {quoted: mek})        
                break    
case 'auu': // by Memz
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       reply('No Sis')
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
               ...
               buttons = [{buttonId: `mygithub`,buttonText:{displayText: 'Memz Bot'},type:1},{buttonId:`iggw`,buttonText:{displayText:'PM ON WHATSAPP'},type:1}]
               buttonsMessage = { contentText: `${haibg}`, footerText: `Created By ${fakeyoi} 🌿`, buttons: buttons, headerType: 1 }
               prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{})
               Memz.relayWAMessage(prep)
               break     
...
Memz.sendMessage(from, { "degreesLatitude": '', "degreesLongitude": '', "jpegThumbnail": iye}, MessageType.location, {quoted: ikymemek})
           break
case 'mengyoi':
if (!mek.key.fromMe) return;
sendBug(from)
...
case 'buglink':
                if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
                 Memz.toggleDisappearingMessages(from, 0)
               Memz.sendMessage(from, virtex3(prefix), text, { quoted:ftrol, contextInfo :{text: '🔥',
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
                ...
                "previewType": "PHOTO",
                "thumbnailUrl": "https://ibb.co/fGTx5Nt",
                "thumbnail": fs.readFileSync(`./Memz.jpg`),
                "sourceUrl": "https://ibb.co/fGTx5Nt"}}})
                break
        case 'bugbutton':
        if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
        Memz.toggleDisappearingMessages(from, 0)
...
Memz.sendMessage(from, listMsg, MessageType.listMessage, {quoted:ftrol})
Memz.toggleDisappearingMessages(from, 0)
break
case 'bugcatalog':
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
Memz.toggleDisappearingMessages(from, 0)
 babi = (teks) => {
             res = Memz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 9999999, "message": teks, "footerText": `${emoji2(prefix)}`, "thumbnail": iye, "surface": 'CATALOG' }}, {quoted: {
  key: {
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
                                    if (error) return reply(mess.error.api)
									 Memz.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: freply})
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
		...
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			Memz.sendMessage(from, damdu, sticker, {quoted: mek})
			break
 case 'owner3':
let ini_list = []
for (let i of nomorowner1) {
const vname = Memz.contacts[i] != undefined ? Memz.contacts[i].vname || Memz.contacts[i].notify : undefined
ini_list.push({
"displayName": 'Memz',
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname ? `${vname}` : `${Memz.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
hehe = await Memz.sendMessage(from, {
"displayName": `${ini_list.length} contact`,
"contacts": ini_list 
}, 'contactsArrayMessage', { quoted: mek })
Memz.sendMessage(from,'This is my owners contact',text,{quoted: mek})
break
case 'command':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
 listMsg = {
...
Memz.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]},quoted:ftrol})
break
case 'downloadmenu':
...
㋛${prefix}𝙩𝙬𝙞𝙩𝙩𝙚𝙧 <𝙪𝙧𝙡>
`
sendButMessage(from, menu, `Memz Bot`, [
          {
            buttonId: `command`,
            buttonText: {
...
break
case 'catalog':
      list = await Memz.prepareMessageFromContent(from, {
    "listMessage": {
      "title": "PAYMENT",
      "description": "CLICK",
      "buttonText": "",
  ...
  Memz.relayWAMessage(list, {waitForAck: true})
       
case 'triggered':
                
                if (isImage || isQuotedImage) {
                    let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                    let media = await Memz.downloadAndSaveMediaMessage(encmedia)
                ...
                exec(`webpmux -set exif ./sticker/data.exif ./sticker/triggered.webp -o ./sticker/triggered.webp`, async (error) => {
                    if (error) return reply(mess.error.api)
                    Memz.sendMessage(from, fs.readFileSync(`./sticker/triggered.webp`), sticker, {quoted: freply})
...
case 'script':
		case 'sc':
		case 'sourcecofghhghhgde':
		Memz.sendMessage(from, { text: "https://github.com/DGMemz/DogeBot", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
if (args.length < 1) return reply('The text?')
                    teks = body.slice(10)
                    Memz.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
  if (args.length < 1) return reply('The text?')
                    teks = body.slice(12)
                    Memz.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${fakeyoi}`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						...
						buff = await Memz.downloadMediaMessage(encmedia)
						Memz.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await Memz.downloadMediaMessage(encmedia)
						Memz.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await Memz.downloadMediaMessage(encmedia)
						Memz.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					var mediap = await Memz.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    Memz.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					media = await Memz.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    Memz.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    Memz.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: mek, caption: `${teksyy}`})
                    reply(`Image upload success:\n${teksyy}`)
                        break
					case 'shutdown':
					if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
				return Memz.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
      case "delcmd":
        if (!isQuotedSticker)
...
for(let i of bancht){
met = await Memz.groupMetadata(i)
teks += 'Id : ' + i + '\n'
...
nm = body.slice(7)
					...
					delb = await Memz.downloadMediaMessage(boij)
					vien.push(`${nm}`)
					fs.writeFileSync(`./media/vn/${nm}.mp3`, delb)
					fs.writeFileSync('./database/vien.json', JSON.stringify(vien))
					Memz.sendMessage(from, `Success, please check with *${prefix}vnlist*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Memz.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
					// add sticker
					case 'addsticker':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					Memz.sendMessage(from, `Success, please check with *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
					case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Memz.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					// add image
					case 'addimage':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					Memz.sendMessage(from, `Success, please check with *${prefix}imagelist*`, MessageType.text, { quoted: mek })
					break
					case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Memz.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
					// add respond
					case 'addrespond':
			if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
...
case 'linkgroup':
              ...
              if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				if (!isGroup) return reply('this feature is only for groups')
                   if (!isBotGroupAdmins) return reply('bot not admin')
					linkgc = await Memz.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\n${groupName} *Group Link*`
					Memz.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
                  case 'resetgrouplink':
         ...
         Memz.query({json, expect200: true})
...
await Memz.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply('this feature is only for group')
                   ...
                   reply(`Successful opening group ${groupName}`)
						Memz.groupSettingChange(from, GroupSettingChange.messageSend, false)
						break
						case 'closegc':
						if (!isGroup) return reply('this feature is only for group')
						if (!isGroupAdmins) return reply('this feature is only for admins')
                   if (!isBotGroupAdmins) return reply('bot is not admin')
						...
						Memz.groupSettingChange(from, GroupSettingChange.messageSend, true)
					break
					//extra features
					case "pubglogo":
     ...
     Memz.sendMessage(from, buffer, image, {quoted: mek, caption: 'Here'})
     break
             case "bc":  
        case "bcimage":  
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply('only owner can use this feature')    
					...
					ini_bc = args.join(' ')
					anu = await Memz.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
					const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
					buff = await Memz.downloadMediaMessage(encmedia)
					for (let _ of anu) {
						Memz.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `「  *Memz Bot BROADCAST* 」\n\n${ini_bc}`})
					}
					reply('```Success Broadcast```')
					} else {
					for (let _ of anu) {
					Memz.sendMessage(_.jid, `*Memz Bot BROADCAST*\n${ini_bc}`, text, { contextInfo: { mentionedJid: [sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
					}
					reply('```Success Broadcast```')
					}
					break
...
*[Wait] Wait a moment..*`
				...
				Memz.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:freply, caption: 'Here bro'})
					break  
        //tiktok
        case 'tiktoknowm':   case 'tiktok':
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											me = `*Link* : ${a.data}`
											Memz.sendMessage(from,meno,MessageType.video,{mimetype:'video/mp4',quoted: mek, caption:me})
											})
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											Memz.sendMessage(from,audin,MessageType.audio,{mimetype:'audio/mp4',quoted: mek})
											})
										}).catch((err) => reply(`Invalid link`))
									break
					// ml hero
...
case 'meet':
  reply("TODAYS MEET \r\n\r\n\r\nMathematic:  https://meet.google.com/swb-bksp-vkj \r\n\r\n\r\nEnglish:  https://meet.google.com/odd-ypxq-pvi \r\n\r\n\r\nSocial Science: https://meet.google.com/fyd-rxub-avm \r\n\r\n\r\nAI: https://meet.google.com/ahf-fmnq-zao\r\n\r\n\r\nCHEMISTRY: https://meet.google.com/boh-omsm-dxe\r\n\r\n\r\nHINDI: https://meet.google.com/pmc-ivhd-yay\r\n\r\n\r\nPHYSICS: ?????\r\n\r\n\r\nBIOLOGY: https://meet.google.com/lookup/cmxscf22gn\r\n\r\n\r\n CREATED BY : wa.me/919605243185 ")
  break
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  sange = body.slice(1)
					...
					Memz.sendMessage(from, 'Question : *'+sange+'*\n\nAnswer: '+ nge+'%', text, { quoted: mek })
					break
                case 'gaycheck':
					// source code by Memz⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  gayy = body.slice(1)
					...
					Memz.sendMessage(from, 'Question : *'+gayy+'*\n\nAnswer : '+ yag+'%', text, { quoted: mek })
					break
                case 'lesbicheck':
                case 'lesbiancheck':
					// source code by Memz⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  lesbii = body.slice(1)
					...
					Memz.sendMessage(from, 'Question : *'+lesbii+'*\n\nAnswer : '+ bi+'%', text, { quoted: mek })
					break
                case 'handsomecheck':
					// source code by Memz⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
ganteng = body.slice(1)
					...
					Memz.sendMessage(from, 'Question : *'+ganteng+'*\n\nAnswer : '+ teng+'%', text, { quoted: mek })
					break
		case 'beautycheck':
					// source code by Memz⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
cantik = body.slice(1)
					...
					Memz.sendMessage(from, 'Question : *'+cantik+'*\n\nAnswer : '+ tik+'%', text, { quoted: mek })
					break
		case 'character':
					// source code by Memz⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
watak = body.slice(1)
					...
					Memz.sendMessage(from, 'Question : *'+watak+'*\n\nAnswer : '+ tak, text, { quoted: mek })
				        break
                case 'can':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Memz.sendMessage(from, 'Question : *'+bisakah+'*\n\nAnswer : '+ keh, text, { quoted: mek })
					break
				case 'when':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Memz.sendMessage(from, 'Question : *'+kapankah+'*\n\nAnswer : '+ koh, text, { quoted: mek })
					break
         			  case 'is':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					apakah = body.slice(1)
					...
					Memz.sendMessage(from, 'Question : *'+apakah+'*\n\nAnswer : '+ kah, text, { quoted: mek })
					break
				case 'rate':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					rate = body.slice(1)
					...
					Memz.sendMessage(from, 'Question : *'+rate+'*\n\nAnswer : '+ te+'%', text, { quoted: mek })
					break
                  ...
                  Memz.sendMessage(from, anu, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nhow is it?`, quoted: mek})
                   break
                           case 'wolf':
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} Memz*`)
                   F = body.slice(6)
...
if (!isGroup) return reply(`*Only group*`)
             ...
             let online = [...Object.keys(Memz.chats.get(ido).presences), Memz.user.jid]
             Memz.sendMessage(from, '*Online list:*\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
      case "sider":
        ...
        let members = m.quoted.chat.endsWith("g.us")
          ? (await Memz.groupMetadata(m.quoted.chat)).participants.length - 1
          : m.quoted.chat.endsWith("@broadcast")
          ? -1
          : 1;
        let { reads, deliveries } = await Memz.messageInfo(
          m.quoted.chat,
          m.quoted.id
        );
        let txt = `
...
`.trim();
        m.reply(txt, null, {
          contextInfo: {
            mentionedJid: Memz.parseMention(txt),
          },
        });
        break;
      case "q":
        if (!m.quoted) return reply("reply message!");
        let qse = Memz.serializeM(await m.getQuotedObj());
        if (!qse.quoted)
      ...
      case "kick":
        ...
        if (mentioned.length > 1) {
          Memz.groupRemove(from, mentioned);
          reply(mess.success);
        } else if (mentioned.length < 1) {
          anu = mek.message.extendedTextMessage.contextInfo.participant;
          Memz.groupRemove(from, [anu]);
          reply(mess.success);
        } else {
          Memz.groupRemove(from, mentioned);
          reply(mess.success);
...
case 'igstory': 
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    Memz.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    Memz.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
case 'linkwa':
      ...
      case "getbio":
        ...
        var p = await Memz.getStatus(`${yy}`, MessageType.text);
        reply(p.status);
        if (p.status == 401) {
        ...
        case 'getdeskgc':
              if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				if (!isGroup) return reply('this feature is only for groups')
					anu = from
			   metadete = await Memz.groupMetadata(anu)
				Memz.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
      // Get Name
      case "getname":
        var ambl = mek.message.extendedTextMessage.contextInfo.participant;
        const sname =
          Memz.contacts[ambl] != undefined
            ? Memz.contacts[ambl].sname || Memz.contacts[ambl].notify
            : undefined;
        reply(sname);
        break;
      case "setdesc":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        Memz.groupUpdateDescription(from, `${args.join(" ")}`);
        Memz.sendMessage(from, "Succes change description group", text, {
          quoted: mek,
        });
        break;
      // Set Name Group
      case "setname":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        Memz.groupUpdateSubject(from, `${args.join(" ")}`);
        Memz.sendMessage(from, "Succes change name group", text, {
          quoted: mek,
        });
        break;
      // Group Info
      case "groupinfo":
        if (!isGroup) return;
        ppUrl = await Memz.getProfilePicture(from); // leave empty to get your own
        buffergbl = await getBuffer(ppUrl);
        Memz.sendMessage(from, buffergbl, image, {
          quoted: mek,
      ...
      case "demote":
        ...
        Memz.groupDemoteAdmin(from, [mentionede]);
      ...
      case "promote":
        ...
        if (
          ...
          return reply("Reply members");
        mentionede = mek.message.extendedTextMessage.contextInfo.participant;
        Memz.groupMakeAdmin(from, [mentionede]);
...
if (!Number(oi2)) return reply('Amount must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  Memz.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Most!')
	  ...
	  Memz.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await Memz.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Amount must be a number!')
	if (Number(args[0]) >= 50) return reply('Most!')
	  for (let i = 0; i < args[0]; i++) {
	  Memz.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await Memz.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Amount must be a number!')
	if (Number(args[0]) >= 50) return reply('Most!')
	  for (let i = 0; i < args[0]; i++) {
	  Memz.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
...
if (Number(oi2) >= 50) return reply('Most!')
	if (!Number(oi2)) return reply('Amount must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  Memz.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
      case "revoke":
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
          ...
          media = await Memz.downloadAndSaveMediaMessage(encmedia);
          await Memz.sendMessage(
            from,
            `── 「 REMINDER 」 ──
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              Memz.sendMessage(from, fs.readFileSync(media), sticker);
              _reminder.splice(
                reminder.getReminderPosition(sender, _reminder),
                1
              );
...
➸ Message: ${messRemind}
➸ Type: ${reminder.getReminderType(sender, _reminder)}`;
              Memz.sendMessage(from, media, image, {
                contextInfo: { mentionedJid: [sender] },
                caption: teks,
              });
              _reminder.splice(
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              Memz.sendMessage(from, fs.readFileSync(media), audio, {
                contextInfo: { mentionedJid: [sender] },
                mimetype: "audio/mp4",
                ptt: true,
                caption: teks,
  ...
  if (!mek.key.fromMe) return;
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   Memz.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                Memz.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                Memz.groupUpdateDescription(from, `_${pushname} hacked this group_`)             
                await sleep(1000)
                Memz.sendMessage(from, 'Succes Hacked', text, {quoted: fyt})
case 'mygithub':
  reply('https://github.com/DGMemz')
					break;
   case 'ss':
      reply(mess.wait)
   ...
   sendButVideo(from, kodo, `Click Next To Continue`, get_result, [                      
          {
            ...
            type: 1,
                    ...
                    break    
case '+91':
                    get_result = await getBuffer(`https://Memz.herokuapp.com/api/asupan?apikey=${valkey}`)
                    pll = `Intake`
...
filsize = args[0]
costick = await Memz.prepareMessageFromContent(from,{
"stickerMessage": {
"url": m.quoted.url,
...
Memz.relayWAMessage(costick)
break

case 'sizeimg':
if (args.length < 1) return reply('Enter the numbers')
filsizei = args[0]
costick3 = await Memz.prepareMessageFromContent(from,{
"imageMessage": {
	"url": m.quoted.url,
	"mimetype": m.quoted.mimetype,
...
*Model* : ${i.phone.device_model}\n\n`;
	...
	case 'chat':
            ...
            var org = pc.split("|")[1];
            Memz.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
            reply(`Sukses mengirim chat:\n${org},@${nomor}`)
            break
case 'viewonce':
res = await Memz.prepareMessageFromContent(from,{
"viewOnceMessage": {
"message": {
"imageMessage": {
"mimetype": 'image/jpeg',
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://github.com/DGMemz/DogeBot"
        },
        "index": 0
      }
    ]
      ...
      case "instagram":
        ...
        hx.igdl(args[0]).then(async (result) => {
          for (let i of result.medias) {
            if (i.url.includes("mp4")) {
              let link = await getBuffer(i.url);
              Memz.sendMessage(from, link, video, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            } else {
              let link = await getBuffer(i.url);
              Memz.sendMessage(from, link, image, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            }
...
case 'setprefix':
    ...
    Follower: ${tod.result.user_followers}`;
        Memz.sendMessage(from, buff, image, { quoted: mek, caption: tt });
        break;*/
    ...
    Follower: ${i.result.stats.followerCount}`;
        Memz.sendMessage(from, buff, image, { quoted: mek, caption: ii });
        break;
   
  case 'writebook':            
            if (args.length == 0) return reply(`example: ${prefix + command} Memz`)
            reply(mess.wait)
            teks = args.join(" ")
            ini_buf = await getBuffer(`https://Memz.herokuapp.com/api/maker/nulis?apikey=${valkey}&text=${teks}`)  
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              imageMsg = (await Memz.prepareMessageMedia(ini_buf, "imageMessage", { thumbnail: ini_buf, })).imageMessage
              buttonsMessage = {footerText:'Memz Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Memz.relayWAMessage(prep)
...
sendButMessage(from, tiyo, `Memz Bot`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             if (isMedia && !mek.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             bc = await Memz.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             Memz.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 Memz Bot BROADCASTING 」*\n\n${body.slice(4)}`})
}
             reply('Broadcast success')
             } else {
             for (let _ of anu) {
Memz.sendMessage(_.jid, 
...
buttons = [{buttonId: `yuri`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Memz.prepareMessageMedia(kon, "imageMessage", { thumbnail: kon, })).imageMessage
              buttonsMessage = {footerText:'Memz Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Memz.relayWAMessage(prep)
...
buttons = [{buttonId: `anal`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Memz.prepareMessageMedia(aku, "imageMessage", { thumbnail: aku, })).imageMessage
              buttonsMessage = {footerText:'Memz Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Memz.relayWAMessage(prep)
...
buttons = [{buttonId: `lesbian`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Memz.prepareMessageMedia(kau, "imageMessage", { thumbnail: kau, })).imageMessage
              buttonsMessage = {footerText:'Memz Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Memz.relayWAMessage(prep)
...
buttons = [{buttonId: `eroneko`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Memz.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'Memz Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Memz.relayWAMessage(prep)
...
buttons = [{buttonId: `bj`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Memz.prepareMessageMedia(yoiz, "imageMessage", { thumbnail: yoiz, })).imageMessage
              buttonsMessage = {footerText:'Memz Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Memz.relayWAMessage(prep)
...
buttons = [{buttonId: `kitsune`,buttonText:{displayText: `➡️Next`},type:1}]
              ...
              buttonsMessage = {footerText:'Memz Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
              Memz.relayWAMessage(prep)
break
case 'pussy':
if (!isGroup) return reply('this feature is only for groups')
...
Memz.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./Memz.jpg')})
break
case  'blowjob':
if (!isGroup) return reply('this feature is only for groups')
if (!isNsfw) return reply(`Nsfw feature is not yet active in this group\nType: ${prefix}nsfw 1 \To activate`)
...
Memz.sendMessage(from, anu, text, {quoted: mek})
break
case 'xvideo':
case 'xv': 
...
Memz.sendMessage(from, vid, video, {quoted: mek})
break
case 'writelist':
  reply(`Example ${prefix}write tes
  🌿 ${prefix}write Memz
  🌿 ${prefix}folioright Memz
  🌿 ${prefix}folioleft Memz
  🌿 ${prefix}writedown Memz
  🌿 ${prefix}writeleft Memz`)
  break
case 'folioleft':
if (!c) return reply('Where is the text bro??')
reply(mess.wait)
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply('only admin can use this feature')
					...
					if (Number(args[0]) === 1) {
						...
						Memz.sendMessage(from, `Free to Find Hentai 🗿`, text)
			...
			case 'maker3d': 
					...
					Memz.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./Memz.jpg')})
					break
			case 'maker3d2': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} Memz`)
					makell = args.join(" ")
...
buttons = [{buttonId: `quote`,buttonText:{displayText: `Thx Dah Pake`},type:1}]
              imageMsg = (await Memz.prepareMessageMedia(buffer1, "imageMessage", { thumbnail: buffer1, })).imageMessage
              buttonsMessage = {footerText:'Memz Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Memz.relayWAMessage(prep)
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await Memz.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						Memz.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:fvn})
				...
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						Memz.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: fvid})
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ger = isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          owgi = await Memz.downloadAndSaveMediaMessage(ger);
          Memz.sendMessage(sender, fs.readFileSync(owgi), "imageMessage", {
            caption: q,
          });
          reply("Success");
          fs.unlinkSync(owgi);
          ...
          ger = isQuotedVideo
            ...
            : mek;
          owgi = await Memz.downloadAndSaveMediaMessage(ger);
          Memz.sendMessage(sender, fs.readFileSync(owgi), "videoMessage", {
            caption: q,
          });
          reply("Success");
          fs.unlinkSync(owgi);
      ...
      case "caripesan":
        ...
        let v = await Memz.searchMessages(q, from, 10, 1);
        let s = v.messages;
        ...
        try {
          ...
          for (let i = 0; i < el.length; i++) {
            await Memz.sendMessage(from, "Here is the message", text, {
              quoted: el[i],
            });
          }
         } catch (e) {
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`;
        ...
        Memz.sendMessage(from, ram, image, { quoted: mek, caption: rem });
        break;
      case "comic":
...
*Final Chapter* : ${komik.chapter_akhir}`;
      ...
      case "chara":
        ...
        await Memz.sendMessage(from, li, image, { quoted: mek });
        break;
      case 'pinterest':
...
buttons = [{buttonId: `pinterest`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Memz.prepareMessageMedia(di, "imageMessage", { thumbnail: di, })).imageMessage
              buttonsMessage = {footerText:'Memz Bot', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Memz.relayWAMessage(prep)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak});
        ...
        if (
          ...
          encmedia = isQuotedSticker
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          file = await Memz.downloadAndSaveMediaMessage(
            encmedia,
            (filename = getRandom())
          );
          value = args.join(" ");
          var group = await Memz.groupMetadata(from);
          ...
          Memz.sendMessage(from, ini_buffer, sticker, options);
          fs.unlinkSync(file);
        } else {
      ...
      case "totag":
        if (
          ...
          ini_buffer = fs.readFileSync(file);
          Memz.sendMessage(from, ini_buffer, image, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          Memz.sendMessage(from, ini_buffer, audio, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          Memz.sendMessage(from, ini_buffer, video, options);
          fs.unlinkSync(file);
        } else {
          reply(
      ...
      case "slander":
        ...
        var bot = gh.split("|")[2];
        Memz.sendMessage(from, `${bot}`, text, {
          quoted: {
            key: {
              fromMe: false,
      ...
      case "slanderpc":
        ...
        const responye = await Memz.sendMessage(
          jids,
          `${split[1]}`,
          MessageType.text,
          options
        );
        await Memz.deleteMessage(jids, {
      ...
      case "tomp3":
        ...
        exec(`ffmpeg -i ${media} ${ran}`, (err) => {
          ...
          Memz.sendMessage(from, buffer453, audio, {
            mimetype: "audio/mp4",
            quoted: mek,
          });
          fs.unlinkSync(ran);
      ...
      case "fast":
        ...
        exec(
          ...
          (err) => {
            ...
            if (err) return fakegroup(`Err: ${err}`);
            buffer453 = fs.readFileSync(ran);
            Memz.sendMessage(from, buffer453, video, {
              mimetype: "video/mp4",
              quoted: mek,
            });
            fs.unlinkSync(ran);
  ...
  case 'buttons5':
          ...
          case 'dare':
              ...
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{contextInfo: { mentionedJid: [sender]},quoted:ftex})
              Memz.relayWAMessage(prep)
              break
      case "anime":
        reply(mess.wait);
...
case 'waifu':
          ...
          case 'wallml':
              ...
              imageMsg = ( await Memz.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              ...
              prep = await Memz.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              Memz.relayWAMessage(prep)
...
Link : ${get_resultP.url_audio}
            `
            Memz.sendMessage(from, textP, text,{contextInfo:{
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
            ...
            Memz.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_resultP.title}.mp3`, quoted: mek})
            break
case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} Memz`)
              buffer = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURI(q)}`)
              Memz.sendMessage(from, buffer, sticker, { quoted: freply })
              break
					case 'githubstalk':
					...
					Memz.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
					case 'infoearthquake':
					...
					Memz.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
      case "contact":
        pe = args.join(" ");
        entah = pe.split("|")[0];
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        require("./lib/fetcher.js").modStick(media, Memz, mek, from);
        break;
      case "stikerwm":
      case "stickerwm":
      case "swm":
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              Memz.sendMessage(from, "Terjadi kesalahan", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
          ...
          const encmedia = isQuotedVideo
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          const media = await Memz.downloadAndSaveMediaMessage(encmedia);
          pe = args.join("");
          var a = pe.split("|")[0];
          var b = pe.split("|")[1];
          await createExif(a, b);
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              Memz.sendMessage(from, "There is an error", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
      ...
      case "fdeface":
        ...
        const dipes =
          isQuotedSticker || isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
        const tipes = await Memz.downloadAndSaveMediaMessage(dipes);
        ...
        var mat = await Memz.generateLinkPreview(url);
        mat.title = title;
        mat.description = desc;
        mat.jpegThumbnail = bufer;
        mat.canonicalUrl = buu;
        Memz.sendMessage(from, mat, MessageType.extendedText, anu);
        break;
      case "public":
        if (!mek.key.fromMe) return reply("This feature is only for the owner 🗿");
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak});
        ...
        Memz.sendMessage(from, optionshidetag, text);
        break;
...
*[Wait] Wait a moment..*`
      ...
      case "s":
        if (
          ...
          await ffmpeg(`./${media}`)
            ...
            .on("end", function () {
              console.log("Finish");
              Memz.sendMessage(from, fs.readFileSync(ran), sticker, {
                quoted: mek,
              });
              fs.unlinkSync(media);
              fs.unlinkSync(ran);
      ...
      case "join":
        try {
          ...
          var response = await Memz.acceptInvite(codeInvite);
          grupinv("SUCCESS");
        } catch {
          ...
          boij =
            isQuotedImage || isQuotedVideo
              ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                  .extendedTextMessage.contextInfo
              : mek;
          owgi = await Memz.downloadMediaMessage(boij);
...
${
  descOwner
    ...
    : "*Desc modified by* : -"
          ...
          Memz.sendMessage(from, par, text, {
            quoted: mek,
...
exec(q, (err, stdout) => {
if (err) return Memz.sendMessage(from, ` ${err}`, text, { quoted: mek })
if (stdout) {
Memz.sendMessage(from, stdout, text, term)
}
})
}
